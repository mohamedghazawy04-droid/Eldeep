...
import { X, Award, Sparkles, User, Phone, MapPin, Check, Gift, ArrowRight, ShieldCheck, Mail, CheckCircle2 } from 'lucide-react';
          ...
          {activeCustomer && !isEditing ? (
                ...
                <div>
                  ...
                  {activeCustomer.email && (
                    <div className="text-xs text-slate-500 dark:text-slate-400 mt-1 flex items-center gap-1.5 font-mono">
                      ...
                      <span className="truncate max-w-[180px] sm:max-w-xs">{activeCustomer.email}</span>
                      {activeCustomer.isEmailVerified ? (
                        <span className="text-[10px] text-emerald-700 dark:text-emerald-300 font-bold bg-emerald-100 dark:bg-emerald-950/80 px-1.5 py-0.5 rounded-md border border-emerald-300 dark:border-emerald-800 flex items-center gap-0.5 shrink-0">
                          <CheckCircle2 className="w-3 h-3 text-emerald-600" />
                          <span>مؤكد ومفعل</span>
                        </span>
                      ) : (
                        <span className="text-[10px] text-amber-700 dark:text-amber-300 font-bold bg-amber-100 dark:bg-amber-950/80 px-1.5 py-0.5 rounded-md border border-amber-300 dark:border-amber-800 shrink-0">
                          في انتظار التأكيد ⏳
                        </span>
                      )}
                    </div>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="p-3 bg-sky-50 dark:bg-sky-950/40 rounded-2xl border border-sky-100 dark:border-sky-900/50 text-xs text-sky-800 dark:text-sky-300">
                💡 <strong>برنامج نقاط الولاء:</strong> سجّل بياناتك لبدء تجميع نقاط الولاء مع كل طلب (كل 100 ج.م = 1 نقطة = 1 ج.م خصم مباشر) واستبدالها بخصومات فورية بالسلة!
              </div>
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">
                  البريد الإلكتروني (لتأكيد الحساب واستلام الإشعارات)
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pr-9 pl-3 py-2.5 bg-slate-100 dark:bg-slate-800 rounded-xl text-xs sm:text-sm font-medium border border-transparent focus:border-sky-500 focus:bg-white dark:focus:bg-slate-900 outline-none transition-colors font-mono text-left"
                    dir="ltr"
                  />
                </div>
              </div>
              ...
              <div className="flex gap-2 pt-2">
                ...
                >
                  {activeCustomer ? 'حفظ التعديلات' : 'تسجيل وتفعيل الحساب'}
                </button>
                {activeCustomer && (
                  <button
                    type="button"
